...
| TD-003 | 2026-09-06 | `Adapter::selftest` re-runs `detect()` internally, so `adapters` spawns each present harness's `--version` twice (shape inherited from pi, copied by omp per the exec-plan). Cold one-shot path; flagged by the post-milestone simplify pass. | Two redundant subprocess spawns per `adapters` run; the trait hides that detection is shared. | When the trait is next widened (Path B), change to `fn selftest(&self, version: Option<&str>)` and let `cmd_adapters` pass its detection down. |
